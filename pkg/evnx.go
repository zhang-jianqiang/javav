...
func SetjavaEnv(path string) error {
	if path == "" {
	...
	for k, v := range envPath {
		if strings.Contains(v, javaPath) || v == "" {
...
func SetLink(target string) (err error) {
	...
	if javaHome == "" {
		return errors.New("JAVA_HOME must be settled")
	}

	err = exec.Command("cmd", "/C", "rmdir", javaHome).Run()
	if err != nil {
		return err
	}

	err = exec.Command("cmd", "/C", "mklink", "/D", javaHome, target).Run()
	if err != nil {
